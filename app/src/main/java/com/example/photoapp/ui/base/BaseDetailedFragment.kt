...
abstract class BaseDetailedFragment : BaseFragment() {
    ...
    protected lateinit var currentItem: BaseResponse
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        menu.findItem(R.id.toolbar_browser).isVisible = checkBrowser()
        menu.findItem(R.id.toolbar_share).isVisible = checkShare()
    }
    ...
    private fun checkBrowser(): Boolean {
        browserIntent =
            Intent(Intent.ACTION_VIEW).apply { data = Uri.parse(currentItem.links.html) }
    ...
    private fun checkShare(): Boolean {
        shareIntent = Intent(Intent.ACTION_SEND).apply {
            putExtra(Intent.EXTRA_TEXT, currentItem.links.html)
            type = "text/html"
        }
